...
$deja= $acteurC->rechercheracteur($login['id_login']);
if($deja!=null){
    echo "<script>alert(\"votre dosier en cours traiter\")</script>";
    ...
    if(!empty($_FILES))
    {
        ...
        $file_des='cv/'.$file_name;

        $ex_auth=array('.pdf', '.PDF');
        if(in_array($file_extension, $ex_auth)){
            if(move_uploaded_file($file_tmp_name, $file_des)){
                echo "<script>alert(\"fichier envoyer\")</script>";
            }
        }else{
            echo "<script>alert(\"fichier non pdf\")</script>";;
        }
        $acteur = new acteur(
            $login['id_login'],
            $file_des,"en attente"
...
<body class="speaker-template-default single single-speaker postid-138 woocommerce-no-js">
	...
	<div class="container">
        ...
        <form action="" method="POST" enctype="multipart/form-data" class="aaa">
        <p style="color:#FFF";>you want to be one of our actors, upload your profile here</p>
